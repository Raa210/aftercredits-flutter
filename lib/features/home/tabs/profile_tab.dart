...
class _ProfileTabState extends State<ProfileTab> {
  ...
  Future<void> _fetchStats() async {
    ...
    final reviews = await _userData.getReviewCount();
    if (mounted) {
      setState(() {
  ...
  Widget _buildStats() {
    return FadeInUp(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            _StatItem(value: '$_watchedCount', label: 'Film\nDitonton'),
            const _StatDivider(),
            _StatItem(value: '$_watchlistCount', label: 'Watchlist'),
            const _StatDivider(),
            _StatItem(value: '$_reviewCount', label: 'Reviews'),
            const _StatDivider(),
            const _StatItem(value: '0', label: 'Mengikuti'),
          ],
        ),
      ),
    );
